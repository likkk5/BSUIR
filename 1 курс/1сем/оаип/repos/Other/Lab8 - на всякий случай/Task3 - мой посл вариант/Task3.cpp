...
void FillEmployees(spisok* femployees, int* famountofem)
{
    ...
    else
    {
        ...
        for(int i=0;i<n;i++)
        {
            ...
            file_load >> femployees[i].hours;
            file_load >> femployees[i].hourly_rate;
            file_load >> femployees[i].salary;
...
void PrintEmployees(spisok* femployees, const int famountofem)
{
    ...
    for (i = 1; i <= famountofem; i++, femployees++)
    {
        printf("| %6d | %31s | %35s | %35s | %17s | %20d | %20.2f | %21.2f |\n",
            femployees->amount, femployees->last_name, femployees->first_name, femployees->midle_name, femployees->personal_number, femployees->hours, femployees->hourly_rate, femployees->salary);
    }
    printf("------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n\n");
}
...
void AddNewEmployee(spisok* femployees, int* famountofem)
{
    ...
    femployees[add].salary = 0.00;
    if (femployees[add].hours > 144)
    {
        femployees[add].salary = (femployees[add].hours - 144) * 2 * femployees[add].hourly_rate + 144 * femployees[add].hourly_rate;
    }
    else
    {
        femployees[add].salary = femployees[add].hours * femployees[add].hourly_rate;
    }
    femployees[add].salary = femployees[add].salary * (1 - 0.12);

    // запись в файл
    std::ofstream file_out("Text.txt", std::ios_base::binary| std::ios_base::app);
    if (!file_out) 
    {
    ...
    {
        file_out << "\n";
        file_out << femployees[add].amount << "\t" << femployees[add].last_name << "\t" << femployees[add].first_name << "\t" << femployees[add].midle_name << "\t" << femployees[add].personal_number << "\t" << femployees[add].hours << "\t" << femployees[add].hourly_rate << "\t" << femployees[add].salary;
    }
    file_out.close();
...
void PrintSearchEmployee(spisok* femployees, const int famountofem, int foundStudent)
{
    ...
    printf("| %6d | %31s | %35s | %35s | %17s | %20d | %20.2f | %21.2f |\n",
        femployees[foundStudent].amount, femployees[foundStudent].last_name, femployees[foundStudent].first_name, femployees[foundStudent].midle_name, femployees[foundStudent].personal_number, femployees[foundStudent].hours, femployees[foundStudent].hourly_rate, femployees[foundStudent].salary);
    printf("------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n\n");
}
...
void DeleteEmployeeByPersonalNumber(spisok* femployees, int* famountofem)
{
        ...
        if (temp_number == deleteNumber)
        {
            ...
            if (*famountofem != 1)
            {
                for (j = i + 1; j < *famountofem; j++)
                {
                    ...
                    femployees[j - 1].hours = femployees[j].hours;
                    femployees[j - 1].hourly_rate = femployees[j].hourly_rate;
                    femployees[j - 1].salary = femployees[j].salary;
                }
            }
            index = i;
            std::cout << index<<"\n";
    ...
    else
    {
        ...
        {
            //file_out.seekp(sizeof(femployees[editIndex]) - editIndex, std::ios_base::beg);
            //file_out.write((char*)&femployees[editIndex], sizeof(femployees[editIndex]));
            //file_out.seekp(countdel, std::ios_base::beg);
            file_out.seekp((index), std::ios_base::beg);
            //file_out << "\n";
            //for (j = index; j < *famountofem - 1 ; j++)
            for (j = i; j < *famountofem - 1; j++)
            {
                file_out << femployees[j].amount << "\t" << femployees[j].last_name << "\t" << femployees[j].first_name << "\t" << femployees[j].midle_name << "\t" << femployees[j].personal_number << "\t" << femployees[j].hours << "\t" << femployees[j].hourly_rate << "\t" << femployees[j].salary << "\n";
            }
        }
...

{
    ...
    femployees[editIndex].salary = 0.00;
    if (femployees[editIndex].hours > 144)
    {
        femployees[editIndex].salary = (femployees[editIndex].hours - 144) * 2 * femployees[editIndex].hourly_rate + 144 * femployees[editIndex].hourly_rate;
    }
    else
    {
        femployees[editIndex].salary = femployees[editIndex].hours * femployees[editIndex].hourly_rate;
    }
    femployees[editIndex].salary = femployees[editIndex].salary * (1 - 0.12);
    ...
    {
        file_out.seekp(editIndex, std::ios_base::beg);
        file_out << femployees[editIndex].amount << "\t" << femployees[editIndex].last_name << "\t" << femployees[editIndex].first_name << "\t" << femployees[editIndex].midle_name << "\t" << femployees[editIndex].personal_number << "\t" << femployees[editIndex].hours << "\t" << femployees[editIndex].hourly_rate << "\t" << femployees[editIndex].salary<< "\n";
    }
    file_out.close();
...
void CalculateSalaryEmployees(spisok* femployees, int* famountofem)
{
    for (int j = 0; j < *famountofem; j++)
    {
        ...
        {
            femployees[j].salary = (femployees[j].hours - 144) * 2 * femployees[j].hourly_rate + 144 * femployees[j].hourly_rate;
        }
        else
        {
            femployees[j].salary = femployees[j].hours * femployees[j].hourly_rate;
        }
        femployees[j].salary = femployees[j].salary * (1 - 0.12);
    }
}
    ...
    do
    {
        ...
        while (true)
        {
            ...
            if (temp_str == "5")
            {
                if (repeat1 != 0)
                {
                    // 5. Удаление данных сотрудника по табельному номеру 
