...
void EditStudent(spisok* femployees, const int famountofem, int editIndex)

{
    ...
    {
        file_out.seekp(sizeof(femployees[editIndex]) * (editIndex - 1), std::ios_base::beg);
...
int main()
{
    ...
    spisok* employees = nullptr;// указатель на массив студентов
    int amountofem; // количество студентов в файле
    int searchEmployee;
    int foundEmployee;
        ...
        while (true)
        {
            ...
            if (temp_str == "6")
            {
                if (repeat1 != 0)
                {
                    ...
                    if (foundEmployee != -1)
                    {
                        EditStudent(employees, amountofem, foundEmployee);
                    }
                    else
                    {
                        printf("\nСотрудник с заданным табельным номером не найден.\n");
