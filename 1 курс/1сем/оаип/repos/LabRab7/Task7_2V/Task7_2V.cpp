...
// проверка: kk-ого бит в бинарном представлении десятичной строки = 0
...
std::string CreateBiDecimal(int n)
{
    ...
    if ((n >= 1) && (n <= Max_N))
    {
        ...
        while (count < n)
        {
            // элементы А = С(сформирован на предыдущем шаге), если k-ый бит в бинарном представлении j-го элемента = 0
            i = 0;
            for (j = 0; j < Num_C; j++)
            {
                //std::cout << "C[" << j << "] = " << C[j] << "\n";
                check = CheckBinaryBit(C[j], k); // проверка: kk-ого бит в бинарном представлении десятичной строки = 0
                if (check)
                {
                    //std::cout << "A[" << i << "] = " << A[i] << "\n";
                    A[i] = "";
            ...
            //формируем новый массив С: первая часть - элементы массива А
            for (j = 0; j < Num_A; j++)
            {
                C[j] = "";
                C[j] = A[j];
            ...
            // формируем новый массив С: вторая часть - элементы массива B
            // элемент массива B - двудесятичное число
            for (j = 0; j < Num_A; j++)
            {
                B[j] = '1' + A[j];
                C[Num_A + j] = "";
