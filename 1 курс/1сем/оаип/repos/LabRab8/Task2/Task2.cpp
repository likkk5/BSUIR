...
#define MAX_COUNT_STUDENT 1000 // максимальное количество студентов в динамическом массиве структур
...
int InputNumber(int ii, int max)
{
    ...
    while (true)
    {
        if (ii == 4)
        {
            printf("Введите номер студента для поиска из отрезка [1..%d]: ", max);
        }
        if (ii == 5)
        {
            printf("Введите номер студента для удаления из отрезка [1..%d]: ", max);
        }
        if (ii == 6)
        {
            printf("Введите номер студента для редактирования из отрезка [1..%d]: ", max);
        }
...
void CreateFileOut(spisok* fstudents, int* famountofst)
{
    ...
    printf("Cписок очередности предоставления мест в общежитии успешно сохранен в файл.\n");
    file_out.close();
}
