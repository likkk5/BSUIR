...
{
    MemoryStorage mem ;
    mem.LoadElf("d:\\study\\4_semestr\\ABC\\acs-spring-l3\\program");
    //std::unique_ptr<IMem> memModelPtr(new UncachedMem (mem));//2907
    std::unique_ptr<IMem> memModelPtr(new CachedMem (mem));//225
    Cpu cpu(*memModelPtr);
    cpu.Reset(0x200);
