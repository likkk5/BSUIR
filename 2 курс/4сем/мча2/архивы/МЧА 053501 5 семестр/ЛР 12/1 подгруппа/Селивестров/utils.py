...
def is_accurate(current, previous, e, trace=False):
    if (len(previous) != 0): 
        e_temp = max([abs(current[i * 2] - previous[i]) for i in range(len(previous))])
...
def clone(object):
    return [subobject for subobject in object]