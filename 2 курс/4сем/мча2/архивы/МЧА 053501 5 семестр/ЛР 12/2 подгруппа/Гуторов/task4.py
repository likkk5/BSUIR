...
y_up = Eq(-k * f(x).diff(x) + 0.5 * f(x), 0)
y_down = Eq(k * f(x).diff(x) + 0.5 * f(x), 0)
...
def get_y_points(points, step):
    ...
    for i in range(1, len(points) - 1):
        ...
        if i - 1 == 0:
            y_diff_1 = (-y_k2 + 4*y_k1 - 3*y_k0) / (2*step)
            system.append(Eq(y_down.lhs.subs(f(x).diff(x), y_diff_1).subs(f(x), y_k1).subs(q, 3.2).subs(k, 0.4), 0))

        if i + 1 == len(points) - 1:
            y_diff_1 = (3*y_k2 - 4 * y_k1 + y_k0) / (2 * step)
            system.append(Eq(y_up.lhs.subs(f(x).diff(x), y_diff_1).subs(f(x), y_k1).subs(q, 12).subs(k, 1.5), 0))
        ...
        if points[i] <= c:
            res = Eq(y_diff_2, func.rhs.subs(f(x).diff(x), y_diff_1).subs(f(x), y_k1).subs(x, points[i]).subs(q, 3.2).subs(k, 0.4))
        else:
            res = Eq(y_diff_2,
                     func.rhs.subs(f(x).diff(x), y_diff_1).subs(f(x), y_k1).subs(x, points[i]).subs(q, 12).subs(k, 1.5))
        system.append(res)
...
def function_to_solve():
    return Eq(f(x).diff(x, x), (-8 * x * (1.5 - 0.5 * x**2) + q * f(x)) / (-k))
...
def get_norm_as_max(y_points, h):
    res = (sum(y ** 2 * h**2 for y in y_points)) ** (1 / 2)
...
def get_number_of_points(precision):
    ...
    plt.plot(x_p_prev, y_p_prev, label=r'f(x) - approximate', color=(0.9, 0.2, 0.6, 0.5))
    # plt.show()
    ...
    plt.plot(x_p_next, y_p_next, label=r'f(x) - approximate', color=(0.9, 0.2, 0.6, 0.5))
...
if __name__ == '__main__':
    ...
    x_p_prev, y_p_prev = get_y_points(x_points, h)
    plt.plot(x_p_prev, y_p_prev, label=r'f(x) - approximate')
    plt.xlabel(r'$x$', fontsize=14)
    plt.ylabel(r'$f(x)$', fontsize=14)
    plt.grid(True)
    plt.legend(loc='best', fontsize=12)
