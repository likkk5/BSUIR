# from sympy import Symbol, diff, sin, cos, series, exp, symbols, Function, Eq, dsolve, linsolve, solve, integrate, isprime
# import numpy as np
#
...
#         x2[y] = float("{0:.3f}".format(x2[y]))
#
#     for y in range(len(y2)):
#         y2[y] = float("{0:.3f}".format(y2[y]))
#     print(x2)
#     print(y2)
#
#     n = 10
...
#         x3[y] = float("{0:.3f}".format(x3[y]))
#     for y in range(len(y3)):
#         y3[y] = float("{0:.3f}".format(y3[y]))
#     print(x3)
#     print(y3)
#
...
#
#
# def get_number_of_points(p_x, f_x, precision):
#     n = 4
#     x_points, h = get_points(n)
#     print(f"Step: {h}")
#
#     x_p_prev, y_p_prev = get_y_points_special_case(x_points, h, -p_x, f_x)
...
#     print(f"Step: {h}")
#
#     x_p_next, y_p_next = get_y_points_special_case(x_points, h, -p_x, f_x)
#     y_next_norm = get_norm_as_max(y_p_next, h)
#
#     while abs(y_next_norm - y_prev_norm) > precision:
#         print(round((x_up - x_down) / (n+2), 10), (x_up - x_down) / (n+2))
#         if round((x_up - x_down) / (n+2), 10) != (x_up - x_down) / (n+2):
#             n += 2
#             continue
...
#     # p_x = (1 + np.cos(variant) * x ** 2) / np.sin(variant)
#     # f_x = -1 / np.sin(variant)
#     # get_y_points_special_case(x_points, h, p_x, f_x)
#     # diff_eq_2 = function_to_solve()
