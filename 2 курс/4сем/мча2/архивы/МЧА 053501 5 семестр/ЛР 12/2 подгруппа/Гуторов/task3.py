...
#
# y_up = 4
# y_down = Eq(2*f(x) - 0.5*f(x).diff(x) - 1, 0)
#
#
# def get_points(n):
#     step = (x_up - x_down) / n
...
#             system.append(Eq(y_down.lhs.subs(f(x).diff(x), y_diff_1).subs(f(x), y_k1), 0))
#
#         if i + 1 == len(points) - 1:
...
#     return Eq(f(x).diff(x, x), (0.5 + 5*x*f(x) + 1.5*f(x).diff(x)))
#
#
# def show_plots(n):
#     n = 50
