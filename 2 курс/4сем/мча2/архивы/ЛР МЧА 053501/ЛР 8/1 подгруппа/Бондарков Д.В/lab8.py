...
# def F(x): return x * np.arctan(x) - math.log(np.sqrt(1 + x**2), math.e)
# def fd1(x): return 1/(1 + x**2)
# def fd2(x): return -2*x/((1 + x**2)**2)
# def fd3(x): return -2 * (1 - 3 * x**2) / ((1 + x**2)**3)
# def fd4(x): return 24*(x - x**3) / ((1 + x**2)**4)
# M2deLR, M4deLR = 0, 0

L, R, DerXdot = -1, 1, 0
def f(x): return 3*np.exp(x)
def F(x): return 3*np.exp(x)
def fd1(x): return 3*np.exp(x)
def fd2(x): return 3*np.exp(x)
def fd3(x): return 3*np.exp(x)
def fd4(x): return 3*np.exp(x)
