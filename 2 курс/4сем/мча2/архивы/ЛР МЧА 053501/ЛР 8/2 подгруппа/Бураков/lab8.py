...
def F(x): return 2 * np.sqrt(x ** 3) / 3
def fd1(x): return 1 / (2 * np.sqrt(x))
def fd2(x): return -1 / (4 * np.sqrt(x**3))
def fd3(x): return 3 / (8 * x * np.sqrt(x**3))
def fd4(x): return -15 * x / (16 * np.sqrt(x**9))
M2deLR, M4deLR = 0, 0
IntEps = 0.000001
IntFormatString = "{:.7f}"
DerEps = 0.01
...
def DerivativeSecondViaTenInMinus4(f, x):
 ...
 return DerivativeSecond(f, x, d)
print()
def delta(derappr): return np.ceil(abs(derappr - fd1(DerXdot)) * (1 / (DerEps
/ 10))) * (DerEps / 10)
...
print()
def delta(derappr): return np.ceil(abs(derappr - fd2(DerXdot)) * (1 / (DerEps
/ 10))) * (DerEps / 10)
...
def IntegralViaRandomSegments(method, f, L, R):
 ...
 while True:
 h_new = h_prev * (LeftCoeff + (RightCoeff - LeftCoeff) *
np.random.rand())
...
def IntegralViaMiddleRectanglesViaEstimation(f, L, R):
 ...
 return IntegralViaRandomSegments(IntegralViaMiddleRectangles, f, L,
R)
...
def delta(intappr): return np.ceil(abs(intappr - intprecised) * (1 / (IntEps
/ 10))) * (IntEps / 10)
intappr = IntegralViaMiddleRectanglesViaEstimation(f, L, R)
print(("ViaMiddleRectangles = " + IntFormatString + " | delta = " +
IntFormatString).format(intappr, delta(intappr)))
intappr = IntegralViaTrapezoidsViaEstimation(f, L, R)
print(("ViaTrapezoids = " + IntFormatString + " | delta = " +
IntFormatString).format(intappr, delta(intappr)))
intappr = IntegralViaSimpsonViaEstimation(f, L, R)
print(("ViaSimpson = " + IntFormatString + " | delta = " +
IntFormatString).format(intappr, delta(intappr)))
print()