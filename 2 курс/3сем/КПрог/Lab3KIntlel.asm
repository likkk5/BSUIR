...
;macro to input string (it's written here in case of possible input)
...
fread macro
    ...
    mov dx, offset buf ;buffer to store read data
    
    mov ah, 3Fh ;read one byte from file 
    int 21h
    jc exit ;if c flag is set -> there's a problem
    ;the number of read bytes is now in ax             
    mov cx, ax ;save the amount of read bytes to cx
...
check_possible_word:
    ...
    mov cl,[buf] ;mov read byte to cl
...
check_word:
    ...
    lea si, word+2 ;load the address of word variable into si
...
while:
    ...
    mov cl, [buf] ;mov read symbol to cl
...
skip:    
    ...
    mov cl, [buf] ;mov read symbol to cl
    cmp al, cl ;check if read symbol is equal to cret
...
word_copy:
    mov bl,0 ; length of the word will be stored here
    mov di, offset word
    mov [di], al           
    inc si                 
    inc di
...
count_rows_end: ;we may be here if we had problems
