...
end_cop:
    ...
    mov ah, 4Eh ; function to find the first file in the current directory
    mov dx, offset Array
    int 21h
    print CrLf
    execute_program
...
SUCCESS:
    ...
    ; Prompt the user to choose whether to run all programs or a specific one
    MOV ah, 09h ; function to print a string
    LEA dx, MENU_MSG ; load the address of the menu message
    INT 21h
...
PRINT_PROGRAM:
    MOV ah, 09h ; function to print a string
    LEA dx, [si] ; load the address of the program name
    INT 21h
    ADD si, cx ; move to the next program name
    INC cx
    CMP cx, [nCount] ; compare with the total number of programs
    JL PRINT_PROGRAM ; loop until all programs are printed
    ...
    ; Check if the user wants to run all programs
    CMP al, 0
    JE RUN_ALL_PROGRAMS

    ; If the user wants to run a specific program
    MOV ah, 09h ; function to print a string
    LEA dx, MENU_MSG ; load the address of the menu message
    INT 21h

    ; Ask the user to enter the program name
    LEA si, line_text ; load the address of the input buffer
    MOV cx, 126 ; maximum length for the program name
    CALL enter_line ; call the macro to get user input

    ; Execute the specified program
    MOV ah, 4Bh ; function to load and run a program
    MOV al, 0 ; load and accomplish    
    LEA bx, line_text ; address of the program name
    MOV dx, offset DTA + 1Eh ; in DTA area, file name is situated at 1Eh offset
    INT 21h

    ; Check if the program execution was successful
    JNC EXECUTION_SUCCESS
    print error_msg
    JMP END_PROGRAM

EXECUTION_SUCCESS:
    ; Ask the user if they want to repeat the selection
    MOV ah, 09h ; function to print a string
    LEA dx, MENU_MSG ; load the address of the menu message
    INT 21h
    ...
    ; Check if the user wants to repeat the selection
    CMP al, 0
    JE END_PROGRAM
...
RUN_ALL_PROGRAMS:
    ; ... (existing code to run all programs)

END_PROGRAM:
    ; ... (existing code to close files and terminate the program)
