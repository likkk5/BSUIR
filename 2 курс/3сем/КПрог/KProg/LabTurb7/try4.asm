...
.data
   ...
   prompt_msg db 0Dh, 0Ah, "Do you want to execute all programs? (y/n): $"
    specific_program_prompt db 0Dh, 0Ah, "Do you want to execute specific program? (y/n): $"
    choose_program_prompt db 0Dh, 0Ah, "Which programm do you want to execute: $" 
...
PRINT_PROG:   
    ...
    mov [nCount], 0
    ; Initialize si to point to the beginning of the buffer
    lea si, [FileBuffer]
print_characters:
    ; Load the character from FileBuffer into al
    mov al, [si]

    ; Check if the character is the null terminator (end of the string)
    cmp al, 0
    je end_print
    ...
    ; Move to the next character in FileBuffer
    inc si
...
end_print:
    ; Print the total count of exe programs
    ;mov ah, 9
    ;lea dx, [nCount]
    ;int 21h
    ; Prompt the user if they want to execute programs
    mov ah, 9
    lea dx, prompt_msg
    int 21h
    ...
    int 21h
    cmp al, 'y'
    je NEW_PROG  ; Jump to NEW_PROG if the user enters 'y'
    cmp al, 'n'
    je ASK_SPECIFIC_PROGRAM  ; Jump to ask for a specific program if the user enters 'n'
    
ASK_SPECIFIC_PROGRAM:
    ; Prompt the user for a specific program
    mov ah, 9
    lea dx, specific_program_prompt
    int 21h
    cmp al, 'y'
    ;je SEARCH 
    cmp al, 'n'
