...
WORD_:  ; Label for handling individual words
pop cx

push cx
mov cx, 00h
cmp ds:[di+1], cx  ; Compare the next character in the buffer with the end-of-string marker
...
jmp WORD_  ; Jump back to the WORD label to continue word comparison
...
push cx
mov cx, '$'
cmp ds:[di], cx  ; Compare the value at memory location DI with '$'
je END_TEST  ; If it's the end marker '$', jump to the end of the test
pop cx
jmp WORD_ ; Otherwise, jump to the label WORD to continue the process
...
jmp END_  ; Jump to the END label

END_:  ; Label marking the end of the program
